...
// MATCH ... AGAINST filters on ds, for every FULLTEXT index those filters can
// use. The returned predicates are redundant - each is entailed by the MATCH
// that produced it - so adding them to the condition set cannot change which
// rows qualify. What it does is let the existing multi-valued index machinery
// recognise an access path, without teaching it anything about full-text
// search: it sees the same `'term' member of (<tokenize expr>)` shape a user
...
func deriveFTSIndexFilters(ds *logicalop.DataSource) []expression.Expression {
	...
	for _, cond := range ds.AllConds {
		...
		if len(matchArgs) != 2 {
			...
			// index; a FULLTEXT index only ever covers one.
			continue
		}
