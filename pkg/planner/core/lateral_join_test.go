...
func TestLateralJoinPlanBuilding(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:              "LATERAL with RIGHT JOIN and outer ref returns error",
			sql:               "SELECT * FROM t RIGHT JOIN LATERAL (SELECT t.a) AS dt ON true",
			expectError:       true,
			expectedErrorCode: 1,
		},
		{
			name:        "LATERAL with RIGHT JOIN without outer ref builds LogicalApply",
...
func TestLateralJoinErrorPaths(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:        "RIGHT JOIN with LATERAL and outer ref is invalid",
			sql:         "SELECT * FROM t RIGHT JOIN LATERAL (SELECT t.a) AS dt ON true",
			expectError: true,
		},
		{
			name:        "RIGHT JOIN with LATERAL without outer ref is valid",
			sql:         "SELECT * FROM t RIGHT JOIN LATERAL (SELECT 1 AS x) AS dt ON true",
			expectError: false,
		},
		{
...
func TestLateralJoinMySQLCompatibility(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// MySQL allows RIGHT JOIN LATERAL
			name:        "RIGHT JOIN LATERAL with constant subquery",
			sql:         "SELECT * FROM t RIGHT JOIN LATERAL (SELECT 1 AS x) AS dt ON true",
			expectError: false,
		},
		{
			// MySQL errors on RIGHT JOIN LATERAL with outer ref (WL#8652 F-4)
			name:        "RIGHT JOIN LATERAL with correlation errors",
			sql:         "SELECT * FROM t RIGHT JOIN LATERAL (SELECT t.a) AS dt ON true",
			expectError: true,
			errorMsg:    "LATERAL cannot be used with RIGHT JOIN",
		},
		{
