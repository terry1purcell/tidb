...
// For TiDB root tasks:
//
//	plan-cost = child-cost + hash-mem-cost + (agg-cost + group-cost) / sqrt(concurrency) + (hash-build-cpu-cost + hash-probe-cost) / concurrency
//
// Aggregation and grouping speed up with concurrency, but far less than linearly.
// A single data-fetcher goroutine calls Next on the child and hands chunks to the
// partial workers (see HashAggExec.fetchChildData), so the workers contend on one
// serialized input path rather than scaling freely with their own count. Measured on
// a 200k-row, 100%-NDV GROUP BY, raising partial concurrency from 1 to 16 cut runtime
// by roughly 1.6x, not 16x. sqrt(concurrency) is a deliberately rough fit to that
// sublinear curve: a full /concurrency overstates the speedup and makes HashAgg look
// cheaper than it runs, while no division at all understates it. Hash-build and
// hash-probe work over the partitioned hash table and do parallelize, so they keep
// the full /concurrency.
//
...
func getPlanCostVer24PhysicalHashAgg(pp base.PhysicalPlan, taskType property.TaskType, option *costusage.PlanCostOption, isChildOfINL ...bool) (costusage.CostVer2, error) {
	...
	if taskType == property.RootTaskType {
		...
		p.PlanCostVer2 = costusage.SumCostVer2(startCost, childCost, hashMemCost,
			costusage.DivCostVer2(costusage.SumCostVer2(aggCost, groupCost), math.Sqrt(concurrency)),
