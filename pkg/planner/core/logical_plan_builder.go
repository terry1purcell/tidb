...
func (b *PlanBuilder) buildLateralJoin(ctx context.Context, leftPlan, rightPlan base.LogicalPlan, joinNode *ast.Join) (base.LogicalPlan, error) {
	...
	case ast.RightJoin:
		// Per MySQL WL#8652 F-4: RIGHT JOIN LATERAL is only allowed when the LATERAL
		// subquery has no FROM-clause outer references. If it has correlated columns
		// referencing the left side, the left side may produce NULLs which the LATERAL
		// subquery cannot safely depend on.
		if len(corCols) > 0 {
			return nil, plannererrors.ErrInvalidLateralJoin.GenWithStackByArgs("LATERAL cannot be used with RIGHT JOIN when it references the left side")
		}
	...
	// Set nullability based on join type
	if joinType == base.LeftOuterJoin {
		...
		util.ResetNotNullFlag(ap.Schema(), 0, leftPlan.Schema().Len())
	}
